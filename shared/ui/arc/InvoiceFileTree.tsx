...
export type InvoiceTreeSelection =
  | { kind: "all" }
...
type Props = {
  ...
  onSelect: (next: InvoiceTreeSelection) => void;
  onCreateFolder: (name: string) => void;
  onRenameFolder: (folderId: string, nextName: string) => void;
  onDeleteFolder: (folderId: string) => void;
};

function isActive(selection: InvoiceTreeSelection, target: InvoiceTreeSelection) {
...
export function InvoiceFileTree({
  invoices,
  folders,
  selection,
  onSelect,
  ...
  const groupedFolders = useMemo(() => {
    ...
    const map = new Map<string, InvoiceFolder[]>();
  ...
  const invoiceCountText = useMemo(() => {
    const total = invoices.filter((i) => i.role === "invoice").length;
    if (selection.kind === "all") return `${total}`;
    const folderCount = invoices.filter(
      (i) => i.role === "invoice" && i.invoiceFolderId === selection.folderId
    ).length;
  ...
  return (
    <div className="p-2">
      <div className="flex items-center justify-between px-2 py-1">
        <div className="text-[11px] font-semibold text-[#4A331D]">
          Invoice folders{" "}
          <span className="text-[10px] font-normal text-[#8B7E6A]">({invoiceCountText})</span>
        </div>
        <button
                        ...
                        <button
                          ...
                          onClick={() => {
                            const ok = window.confirm(
                              `Delete folder "${f.name}"?\n\nInvoices will remain (not deleted).`
                            );
                            if (!ok) return;
                            onDeleteFolder(f.id);
                          }}
